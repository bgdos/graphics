...
function stickyTableHeader(e, height)
{
    ...
    {
        if (ths[i].clientWidth > tbtd[i].clientWidth)
        {
            var w = getComputedStyle(ths[i]).getPropertyValue("width");
            setCSS(tbtd[i], {width: w});
        }
        else
        {
            var w = getComputedStyle(tbtd[i]).getPropertyValue("width");
            setCSS(ths[i], {width: w});
        }
    }
    width +=  thead.clientWidth;
    setCSS(tb, {height: height, width: width + 'px'});// add the height and the width of the tbody
}
/** search on table **/
function searchTable(tb)
{
    ...
    for (var i = 0; i < trs.length; i++)
    {
        ...
        if (m != -1)
        {
            ...
            {
                if (trs[i].children[j].clientWidth < ths[j].clientWidth)
                    trs[i].children[j].style.width = getComputedStyle(ths[j]).getPropertyValue("width");
                else
                    ths[j].style.width = getComputedStyle(trs[i].children[j]).getPropertyValue("width");
            }
        }  
        else
        {
